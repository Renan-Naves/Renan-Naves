...
//
// Pulls ad-level spend, impressions, and clicks from Meta Marketing API v22
// for the configured ad account and UPSERTs them into the `ad_spend` table
// (one row per ad per day). Called hourly by the Cloudflare cron Worker in
// `cron-worker/` (see docs/ad-spend-sync.md). The `/dashboard` and `/dash`
// pages read ad_spend directly — they never hit this endpoint in the
// request path.
//
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // level=ad: one row per ad per day. The dashboard's "CPL por criativo"
    // matches ad_name against the lead's utm_content, so ad-level granularity
    // is required. Campaign totals are still derivable (GROUP BY campaign_id).
    const url = `https://graph.facebook.com/v22.0/act_${accountId}/insights` +
      `?fields=campaign_id,campaign_name,ad_id,ad_name,spend,impressions,clicks,account_currency,date_start` +
